...
void main( void ) {
	...
	vec3 suncolor  = vec3(1.0,1.0,0.0) - vec3(0.0,0.5,0.0) *abs(sin(t))/**-cos(t)*/;
	vec3 mooncolor = vec3(0.7,0.7,0.7);
	
	//---------------------SKY
	//if(position.y > 0.436)
		gl_FragColor = vec4(0.0,0.4,0.5,1.0)*max(0.0,-cos(t)) + vec4(0.0,0.1,0.2,1.0) +  //Blue Sky
		vec4(1.0,0.0,0.0,1.0)*abs(sin(t))*-cos(t) + //Red Sky
		vec4(1.0,1.0,1.0,1.0)*smoothstep(0.99,1.0,sin(position.x*150.0)*sin(position.y*150.0))*max(0.0,cos(t)); //Stars
	
	//---------------------SUN & MOON
	float phase = mod(floor((t-1.5)/6.28318530718),6.0)-3.0;
	gl_FragColor += vec4(suncolor,1.0) * max(-1000.0*pow(pow((position.x - 0.5 + sin(t)*0.5)*ratio,2.0) + pow(position.y - 0.3 + cos(t)*0.5,2.0),1.5) + 1.5,0.0); //Sun
	if (pow((position.x - 0.5 + -sin(t)*0.5)*ratio,2.0) + pow(position.y - 0.3 + -cos(t)*0.5,2.0) < 0.002){
		gl_FragColor = vec4(mooncolor,1.0); //Moon
		if (pow((position.x - 0.5 + -sin(t)*0.5)*ratio-0.022*phase,2.0) + pow(position.y - 0.3 + -cos(t)*0.5 -0.022*phase,2.0) < 0.002)
			gl_FragColor = vec4(0.0,0.1,0.2,1.0);
	}
/*
        // --------------------Ocean
	if ( position.y < 0.436) {
		vec2 p = vec2(position.x+sin(time+position.y*55.0)*0.01,(0.436-position.y)*0.3+0.436);
		
		gl_FragColor = vec4(0.0,0.0,0.0*max(0.0,-cos(t)),1.0) * max(0.5,cos ((position.y+sin(time/2.0+position.x*3.0))));
		gl_FragColor += vec4(suncolor/2.0,1.0) * max(-1000.0*pow(pow((p.x - 0.5 + sin(t)*0.5)*ratio,2.0) + pow(p.y - 0.3 + cos(t)*0.5,2.0),1.5) + 1.5,0.0);
	}
*/
	...
	float f9 = pow(octave(vec2(position.x + time * 0.005, position.y + time * 0.001), 8, 0.75) * 0.5 + 0.5, 1.0);
/*
